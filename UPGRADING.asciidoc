...
The network interface which Elasticsearch binds to (`elasticsearch_network_host`) has been changed to localhost (i. e. `127.0.0.1` or `::1`); see https://www.elastic.co/guide/en/elasticsearch/reference/2.0/breaking_20_network_changes.html#_bind_to_localhost[Network changes/Bind to localhost].
...
The web interface has been integrated into the Graylog server and was rewritten in React. Therefore configuring it has changed fundamentally since the last version(s). Please consult the http://docs.graylog.org/en/2.0/pages/configuring_webif.html[Graylog documentation] for details.
