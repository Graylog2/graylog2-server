...
public class MoreSearchAdapterOS2 implements MoreSearchAdapter {
    ...
    public MoreSearch.Histogram eventHistogram(String queryString, AbsoluteRange timerange, Set<String> affectedIndices,
        ...
        try {
            ...
            // in case of an invalid query entered in the search box, we want to return an empty result set
            if(ex.getCause() != null) {
                for(Throwable suppressed : ex.getCause().getSuppressed()) {
                    if(suppressed.getMessage().contains("Failed to parse query")) {
    ...
    QueryBuilder createQuery(String queryString, TimeRange timerange, Set<String> eventStreams, String filterString, SourceStreamFilter sourceStreamFilter, Map<String, Set<String>> extraFilters,
                             Set<String> excludedEventIds) {
