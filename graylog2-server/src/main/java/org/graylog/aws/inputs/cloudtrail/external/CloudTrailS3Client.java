...
public class CloudTrailS3Client {
    ...
    public String readCompressed(String bucket, String key) throws IOException {
        ...
        try {
            ...
            if (objectBytes == null) {
                String errorMessage = "Received null response when fetching S3 object from bucket [" + bucket
                        + "], key [" + key + "].";
            ...
            String errorMessage = f("Failed to read or decompress S3 object from bucket [%s], key [%s]: %s", bucket,
                    key, e.getMessage());
            inputFailureRecorder.setFailing(getClass(), errorMessage);
