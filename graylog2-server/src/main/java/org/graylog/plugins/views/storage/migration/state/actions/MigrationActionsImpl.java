...
public class MigrationActionsImpl implements MigrationActions {
    ...
    public void provisionDataNodes() {
        ...
        activeDataNodes.values().stream()
                ...
                .forEach(node -> {
                    try {
                        dataNodeCommandService.triggerCertificateRequest(node.getNodeId());
    ...
    public void startDataNodes() {
        nodeService.allActive().values().stream().filter(node -> node.getDataNodeStatus() != DataNodeStatus.AVAILABLE)
                .forEach(node -> {
                    try {
