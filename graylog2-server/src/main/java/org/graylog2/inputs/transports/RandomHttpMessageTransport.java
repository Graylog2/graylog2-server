...
public class RandomHttpMessageTransport extends RandomMessageTransport {
    @AssistedInject
    public RandomHttpMessageTransport(@Assisted Configuration configuration, EventBus eventBus, ObjectMapper objectMapper) {
        super(configuration, eventBus, objectMapper);

        generator = new FakeHttpRawMessageGenerator(configuration);
    }

    @FactoryClass
      public interface Factory extends Transport.Factory<RandomHttpMessageTransport> {
        @Override
        RandomHttpMessageTransport create(Configuration configuration);
