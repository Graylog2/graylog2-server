...
public class DnsLookupAdapterConfiguration implements PluginConfigBean {
    ...
    @Documentation("Number of DNS resolvers in the pool shared by all DNS lookup adapters.")
    @Parameter(value = RESOLVER_POOL_SIZE, validators = PositiveIntegerValidator.class)
    private int poolSize = DEFAULT_POOL_SIZE;

    @Documentation("Interval in which the resolvers in the DNS lookup adapter pool are recreated to cycle in new source ports. Default: 300s")
