...
public class NotificationServiceImpl extends PersistedServiceImpl implements NotificationService {
    ...
    public boolean fixed(NotificationImpl.Type type, Node node) {
        ...
        if (removed) {
            auditEventSender.success(AuditActor.system(), AuditEventTypes.SYSTEM_NOTIFICATION_DELETE, Collections.singletonMap("notification_type", type.getClass().getCanonicalName()));
        }
        return removed;
    }
    ...
    public boolean publishIfFirst(Notification notification) {
        ...
        try {
            save(notification);
            auditEventSender.success(AuditActor.system(), AuditEventTypes.SYSTEM_NOTIFICATION_CREATE, notification.asMap());
        } catch(ValidationException e) {
            // We have no validations, but just in case somebody adds some...
            LOG.error("Validating user warning failed.", e);
            auditEventSender.failure(AuditActor.system(), AuditEventTypes.SYSTEM_NOTIFICATION_CREATE, notification.asMap());
            return false;
        }
