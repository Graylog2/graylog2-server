...
class InputAssociatedStreamsDescriptorTest {

    private MoreSearch moreSearch;
    private SearchUser searchUser;
    private InputAssociatedStreamsDescriptor descriptor;

    @BeforeEach
    void setUp() {
        moreSearch = mock(MoreSearch.class);
        searchUser = mock(SearchUser.class);
        descriptor = new InputAssociatedStreamsDescriptor(moreSearch, Duration.ofMinutes(5));
    }

    @Test
    void compute_returnsStreamIdLists() {
        ...
        final List<EntityMetric<List<String>>> result = descriptor.compute(List.of("input-1", "input-2"));
        ...
        assertThat(result).filteredOn(m -> m.entityId().equals("input-1"))
                ...
                .satisfies(v -> assertThat((List<String>) v).containsExactlyInAnyOrder("stream-a", "stream-b"));
        assertThat(result).filteredOn(m -> m.entityId().equals("input-2"))
                .first().extracting(EntityMetric::value)
                .satisfies(v -> assertThat((List<String>) v).containsExactlyInAnyOrder("stream-c"));
    }

    @Test
    void compute_returnsEmptyListForMissingEntities() {
        when(moreSearch.aggregateGroupedTerms(
                anyString(), any(RelativeRange.class), any(SourceStreamFilter.class),
                anyString(), anyString(), anyInt(), anyInt()))
                .thenReturn(Map.of());

        final List<EntityMetric<List<String>>> result = descriptor.compute(List.of("input-1"));
    ...
    void computeForUser_filtersToPermittedStreams() {
        ...
        final List<String> streamIds = List.of("stream-a", "stream-b", "stream-c");
        final List<String> filtered = descriptor.computeForUser(streamIds, searchUser);

        assertThat(filtered).containsExactlyInAnyOrder("stream-a", "stream-c");
    }

    @Test
    void computeForUser_returnsEmptyListWhenNoStreamsPermitted() {
        when(searchUser.canReadStream("stream-a")).thenReturn(false);
        when(searchUser.canReadStream("stream-b")).thenReturn(false);

        final List<String> filtered = descriptor.computeForUser(List.of("stream-a", "stream-b"), searchUser);
