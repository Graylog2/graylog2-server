...
public class ClusterLookupTableResourceTest {
    ...
    public void performPurge_whenAllCallsSucceedThen200WithPerNodeDetailsReturned() throws Exception {
        ...
        mockSuccessfulCall("testName", "testKey", remoteLookupTableResource1);
        mockSuccessfulCall("testName", "testKey", remoteLookupTableResource2);

        // when
        final javax.ws.rs.core.Response res = underTest.performPurge("testName", "testKey");
        final Map<String, String> resBody = (Map<String, String>)res.getEntity();
        ...
        assertThat(resBody.get("node_1")).isEqualTo("SUCCESS");
        assertThat(resBody.get("node_2")).isEqualTo("SUCCESS");
    }

    @Test
    public void performPurge_whenAtLeastOneHttpCallFailsThen500WithPerNodeDetailsReturned() throws Exception {
        // given
        when(nodeService.allActive()).thenReturn(nodeMap());
        mockSuccessfulCall("testName", "testKey", remoteLookupTableResource1);
        mockFailedCall("testName", "testKey", remoteLookupTableResource2);
        ...
        assertThat(resBody.get("node_1")).isEqualTo("SUCCESS");
        assertThat(resBody.get("node_2")).isEqualTo("ERROR");
    }

    @Test
    public void performPurge_whenAtLeastOneCallFailsWithExceptionThen500WithPerNodeDetailsReturned() throws Exception {
        // given
        when(nodeService.allActive()).thenReturn(nodeMap());
        mockSuccessfulCall("testName", "testKey", remoteLookupTableResource1);
    ...
    private void mockSuccessfulCall(String tableName, String key,
                                    RemoteLookupTableResource remoteLookupTableResource) throws IOException {
        final Call<LookupTableCachePurgingResponse> call = mock(Call.class);

        when(remoteLookupTableResource.performPurge(tableName, key))
                .thenReturn(call);

        when(call.execute()).thenReturn(Response.success(LookupTableCachePurgingResponse.success()));
    }

    private void mockFailedCall(String tableName, String key,
                                RemoteLookupTableResource remoteLookupTableResource) throws IOException {
        final Call<LookupTableCachePurgingResponse> call = mock(Call.class);
        ...
        when(call.execute()).thenReturn(Response.error(500,
                ResponseBody.create(MediaType.parse("plain/txt"), "Error")));
    }
