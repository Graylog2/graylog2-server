...
public class MongoIndexSetRegistryTest {
    ...
    public void isCurrentWriteIndexOnManyIndexSetConfigsNoneFoundDueToMismatchingIndexName() {
        ...
        final ImmutableSet.Builder<IndexSet> mockedMongosBuilder = ImmutableSet.builder();
        final AtomicReference<IndexSet> matchingMongoMock = new AtomicReference<>();
        when(mongoIndexSetFactory.create(any(IndexSetConfig.class)))
                .thenAnswer((Answer<IndexSet>) invocation -> {
                            Object[] args = invocation.getArguments();
                            final IndexSetConfig cfg = (IndexSetConfig) args[0];
                    final IndexSet mockedIndexSet = mock(IndexSet.class);
        ...
        final ImmutableSet<IndexSet> mockedMongos = mockedMongosBuilder.build();
