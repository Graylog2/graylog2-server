...
class OrphanedTokenCleanerTest {
    ...
    private OrphanedTokenCleaner orphanedTokenCleaner;

    @BeforeEach
    void setUp() {
        orphanedTokenCleaner = new OrphanedTokenCleaner(tokenService, auditEventSender, nodeId);
        mocks = new Object[]{tokenService, auditEventSender, nodeId};
        lenient().when(nodeId.getNodeId()).thenReturn("nodeId1");
    }

    @Test
    void doNothingIfNoTokenAreOrphaned() {
        when(tokenService.findOrphanedTokens()).thenReturn(Collections.emptyList());

        orphanedTokenCleaner.doRun();

        verify(tokenService).findOrphanedTokens();
        verifyNoMoreInteractions(mocks);
    }

    @Test
    void sendAuditEventIfOrphanedTokenIsDeletedSuccessfully() {
        final int id = 1;
        when(tokenService.findOrphanedTokens()).thenReturn(List.of(mkToken(id)));
    ...
    @Test
    void sendAuditEventIfOrphanedTokenFailsToBeDeleted() {
        final int id = 2;
    ...
    private AccessTokenService.ExpiredToken mkToken(int id) {
        return new AccessTokenService.ExpiredToken(String.valueOf(id), "token" + id, baseDateTime.plusDays(id), null, "username" + id);
    }
    ...
    private Map<String, Object> expectedContext(int id, String errorMsg) {
        ...
        ctx.put(AccessTokenImpl.NAME, "token" + id).put("userId", "null").put("username", "username" + id);
        if (errorMsg != null) {
