...
export const alice = User.builder()
  ...
  .preferences({ updateUnfocussed: false, enableSmartSearch: true, themeMode: 'dark' })
...
export const sam = User.builder()
  .id('sam-id')
  .username('sam')
  .fullName('Sam Samuelson')
  .firstName('Sam')
  .lastName('Samuelson')
  .email('sam@example.org')
  .permissions(Immutable.List(readerPermissions('sam')))
  .grnPermissions(Immutable.List())
  .preferences({ updateUnfocussed: false, enableSmartSearch: true, themeMode: 'dark' })
  .roles(Immutable.Set(['Reader']))
  .readOnly(true)
...
export const carol = User.builder()
  .id('sam-id')
  .username('sam')
  .fullName('Sam Samuelson')
  .firstName('Sam')
  ...
  .external(true)
  .sessionActive(false)
  .startpage({ type: 'search', id: 'default' })
  .sessionTimeoutMs(10000000000)
  .clientAddress('172.0.0.1')
  .accountStatus('enabled')
  .build();

export const serviceUser = User.builder()
  .id('service-id')
  .username('service')
  .fullName('Service account')
  .firstName('Service')
  .lastName('Account')
  .email('service@account.org')
  .permissions(Immutable.List(readerPermissions('service')))
  .grnPermissions(Immutable.List())
  .preferences({ updateUnfocussed: false, enableSmartSearch: true, themeMode: 'dark' })
  .roles(Immutable.Set(['Reader']))
  .readOnly(true)
