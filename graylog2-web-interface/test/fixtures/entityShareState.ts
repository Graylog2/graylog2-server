...
// grantees
export const everyone = Grantee.builder().id('grn::::user:everyone-id').title('Everyone').type('global').build();

export const security = Grantee.builder()
  .id('grn::::team:security-team-id')
  .title('Security Team')
  .type('team')
  .build();

export const alice = Grantee.builder().id('grn::::user:alice-id').title("Alice Muad'Dib").type('user').build();

export const bob = Grantee.builder().id('grn::::user:bob-id').title('Bob Bobson').type('user').build();

export const john = Grantee.builder().id('grn::::user:john-id').title('John Wick').type('user').build();

export const jane = Grantee.builder().id('grn::::user:jane-id').title('Jane Doe').type('user').build();
...
// active shares
const janeIsOwner = ActiveShare.builder().grant('grant-id').grantee(jane.id).capability(owner.id).build();
...
const missingDependecy = SharedEntity.builder()
...
export const missingDependencies: MissingDependencies = Immutable.Map({
  [security.id]: Immutable.List([missingDependecy]),
});
...
export const failedEntityShareState = EntityShareState.builder()
...
const entityShareState = EntityShareState.builder()
  .entity('grn::::dashboard:dashboard-id')
  .availableGrantees(availableGrantees)
  .availableCapabilities(availableCapabilities)
  .activeShares(activeShares)
