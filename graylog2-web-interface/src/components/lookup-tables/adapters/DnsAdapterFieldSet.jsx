...
class DnsAdapterFieldSet extends React.Component {
  ...
  _updateCacheTTLOverride = (value, unit, enabled, field_prefix) => {

    const config = ObjectUtils.clone(this.props.config);

    // If Cache TTL Override box is checked, then save the value. If not, then do not save it.
    if (enabled && value) {
      config[field_prefix] = enabled && value ? parseInt(value) : null;
      config[field_prefix + '_enabled'] = enabled;
    }
    else {
      config[field_prefix] = null;
      config[field_prefix + '_enabled'] = false;
    }

    config[`${field_prefix}_unit`] = enabled ? unit : null;
    this.props.updateConfig(config);
  };
