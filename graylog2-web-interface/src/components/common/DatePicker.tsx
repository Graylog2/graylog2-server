...
const DatePicker = ({ date, fromDate, onChange, showOutsideDays }: Props) => {
  if (!isValidDateProp(date)) {
    throw Error(
      `Date time provided for date picker "${date}" is not valid. The expected format is ${DATE_TIME_FORMATS.date}.`,
    );
  }
  ...
  return (
    <StyledDayPicker
      initialMonth={date ? toDateObject(date).toDate() : undefined}
      onDayClick={onChange}
      modifiers={modifiers}
