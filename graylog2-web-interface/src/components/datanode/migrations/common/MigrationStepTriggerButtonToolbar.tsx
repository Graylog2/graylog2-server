...
const getSortedNextSteps = (nextSteps: MigrationActions[]) =>
  nextSteps.reduce((sortedNextSteps, step) => {
...
const getTelemetryEvent = (state: MigrationStateItem, step: MigrationActions): TelemetryEventType => {
  switch (state) {
    case 'MIGRATION_WELCOME_PAGE':
      if (step === 'SHOW_MIGRATION_SELECTION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.WELCOME_GO_TO_MIGRATION_STEPS_CLICKED;
      if (step === 'SHOW_RENEWAL_POLICY_CREATION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.WELCOME_CONFIGURE_CERTIFICATE_RENEWAL_POLICY_CLICKED;

      return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.WELCOME_NEXT_CLICKED;
    case 'CA_CREATION_PAGE':
      if (step === 'SHOW_MIGRATION_SELECTION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.CA_GO_TO_MIGRATION_STEPS_CLICKED;
      if (step === 'SHOW_RENEWAL_POLICY_CREATION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.CA_CONFIGURE_CERTIFICATE_RENEWAL_POLICY_CLICKED;

      return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.CA_NEXT_CLICKED;
    case 'RENEWAL_POLICY_CREATION_PAGE':
      if (step === 'SHOW_MIGRATION_SELECTION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.CR_GO_TO_MIGRATION_STEPS_CLICKED;
    ...
    case 'EXISTING_DATA_MIGRATION_QUESTION_PAGE':
      if (step === 'SKIP_EXISTING_DATA_MIGRATION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.REMOTEREINDEX_MIGRATE_EXISTING_DATA_QUESTION_SKIP_CLICKED;
    ...
    case 'MIGRATE_EXISTING_DATA':
      if (step === 'START_REMOTE_REINDEX_MIGRATION')
        return TELEMETRY_EVENT_TYPE.DATANODE_MIGRATION.REMOTEREINDEX_MIGRATE_EXISTING_DATA_START_CLICKED;
...
const MigrationStepTriggerButtonToolbar = ({
  nextSteps = [],
  disabled = false,
  onTriggerStep,
  args = {},
  ...
  return (
    <StyledButtonToolbar>
      {getSortedNextSteps(nextSteps).map((step, index) => (
        <Button
          key={step}
