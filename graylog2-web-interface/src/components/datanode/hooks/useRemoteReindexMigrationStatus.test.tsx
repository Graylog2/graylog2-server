...
const mockOnTrigger = (migrationState: MigrationState = {} as unknown as MigrationState) =>
  jest.fn(() => Promise.resolve(migrationState));

describe('useRemoteReindexMigrationStatus', () => {
  it('should not fetch migration status on other steps', async () => {
    const currentStep = {
      state: MIGRATION_STATE.REMOTE_REINDEX_WELCOME_PAGE.key,
      next_steps: ['REQUEST_MIGRATION_STATUS', 'RETRY_MIGRATE_EXISTING_DATA'] as MigrationActions[],
    };
  ...
  it('should only fetch migration status on the RemoteReindexRunning step', async () => {
    const currentStep = {
      state: MIGRATION_STATE.REMOTE_REINDEX_RUNNING.key,
      next_steps: ['REQUEST_MIGRATION_STATUS', 'RETRY_MIGRATE_EXISTING_DATA'] as MigrationActions[],
    };
  ...
  it('should update nextSteps when migration progress is 100% and status FINISHED', async () => {
    ...
    await waitFor(() => expect(result.current.migrationStatus).toEqual(migrationState.response));
    await waitFor(() =>
      expect(result.current.nextSteps).toEqual(['RETRY_MIGRATE_EXISTING_DATA', 'SHOW_ASK_TO_SHUTDOWN_OLD_CLUSTER']),
    );
  });
