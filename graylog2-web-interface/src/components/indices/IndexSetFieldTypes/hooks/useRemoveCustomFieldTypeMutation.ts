...
const useRemoveCustomFieldTypeMutation = (params: {
  onErrorHandler: (response: RemovalResponse) => void;
  onSuccessHandler: (response: RemovalResponse) => void;
}) => {
  ...
  const put = useMutation(putFieldType, {
    ...
    onSuccess: (response: RemovalResponseJSON) => {
      ...
      const mappedResponse: RemovalResponse = mapValues(
        response,
        ({ successfully_performed, succeeded, errors, failures: failuresJSON }, id) => {
          const failures = failuresJSON.map(({ entity_id, failure_explanation }) => ({
            entityId: entity_id,
