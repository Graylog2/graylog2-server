...
describe('IndexSetFieldTypesList', () => {
  ...
  describe('Shows expanded row on origin bage click', () => {
    it('for origin index', async () => {
      ...
      expect(tableRow).toHaveTextContent('Field type mapping field: Boolean comes from the search engine index mapping. It could have been created dynamically, set by Graylog instance or come from historical profiles and/or custom mappings.');
    });
    ...
    it('for origin profile', async () => {
      ...
      expect(tableRow).toHaveTextContent('Field type mapping field-3: String type comes from profile Profile-1. It overrides possible mappings from the search engine index mapping, either immediately (if index was rotated) or during the next rotation');
    });
    ...
    it('for origin overridden index', async () => {
      ...
      expect(tableRow).toHaveTextContent('Field type mapping field-1: Boolean comes from the individual, custom field type mapping. It overrides possible mappings from the search engine index mapping, either immediately (if index was rotated) or during the next rotation.');
    });
    ...
    it('for origin overridden profile', async () => {
      ...
      expect(tableRow).toHaveTextContent('Field type mapping field-2: Boolean comes from the individual, custom field type mapping. It overrides not only possible mappings from the search engine index mapping, but also mapping field-2: Boolean present in profile Profile-1');
    });
  });
});
