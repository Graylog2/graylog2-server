...
const EventsTbody = styled.tbody(
  ({ expanded, theme }) => css`
...
class Events extends React.Component {
  ...
  renderEvent = (event) => {
    ...
    return (
      <EventsTbody key={event.id} expanded={expanded.includes(event.id)}>
        <CollapsibleTr
          className={event.priority === EventDefinitionPriorityEnum.HIGH ? 'bg-danger' : ''}
        ...
        >
          ...
          <td>{this.renderLinkToEventDefinition(event, eventDefinitionContext)}</td>
          <td>
  ...
  render() {
    ...
    const offsetLimitError =
      paginationQueryParameter.page * paginationQueryParameter.pageSize > EVENTS_MAX_OFFSET_LIMIT;

    const emptyListComponent = query ? (
      <NoSearchResult>No {entity} found for the current search criteria.</NoSearchResult>
    ) : (
      <NoEntitiesExist>No {entity} exist.</NoEntitiesExist>
    );
    ...
    const offsetLimitErrorComponent = (
      <tbody>
        <tr>
          <td colSpan={5}>
            <NoSearchResult>
              Unfortunately we can only fetch Events with an Offset (page number * rows per page) less than or equal to:
              [10000]. Please use more advanced methods (Search Field and Date Filter) in order to get distant chunks of
              results.
            </NoSearchResult>
          </td>
        </tr>
      ...
      <Row>
        <Col md={12}>
          ...
          {eventList.length === 0 && !offsetLimitError ? (
            ...
            <EventListContainer>
              <PaginatedList totalItems={totalEvents} onChange={onPageChange} pageSizes={PAGE_SIZES}>
                <EventsTable id="events-table">
                  ...
                  {offsetLimitError ? offsetLimitErrorComponent : eventList.map(this.renderEvent)}
                </EventsTable>
              </PaginatedList>
            </EventListContainer>
          )}
