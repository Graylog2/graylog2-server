...
const PipelineConnectionsForm = ({ pipeline, connections, streams, save }: Props) => {
  ...
  const notConnectedStreams = useMemo(
    () => streams.filter((s) => !connectedStreams.some((cs) => cs.value.toLowerCase() === s.id.toLowerCase())),
  ...
  return (
    <span>
      ...
      <BootstrapModalForm
        ...
        title={
          <span>
            Edit connections for <em>{pipeline.title}</em>
          </span>
        }
      ...
      >
        <fieldset>
          <FormGroup id="streamsConnections">
            ...
            <SelectableList
              options={formatStreams(notConnectedStreams)}
