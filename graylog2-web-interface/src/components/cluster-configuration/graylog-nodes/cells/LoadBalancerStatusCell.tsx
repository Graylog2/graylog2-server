...
const LoadBalancerStatusCell = ({ node }: Props) => {
  const status = node.lb_status?.toUpperCase();
  ...
  return (
    <MetricsColumn>
      <MetricsRow>
        <Label bsStyle={status === 'ALIVE' ? 'success' : 'warning'} size="xs" aria-label={`Load balancer ${status}`}>
          {status}
        </Label>
      </MetricsRow>
    </MetricsColumn>
...
export default LoadBalancerStatusCell;
