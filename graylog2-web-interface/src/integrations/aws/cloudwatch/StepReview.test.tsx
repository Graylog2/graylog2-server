...
describe('<StepReview>', () => {
  ...
  it('renders the single DynamoDB table state tracking review item', async () => {
    asMock(fetch).mockReturnValueOnce(Promise.resolve({ title: 'Test', configuration: {} }));

    render(
      <ApiContext.Provider value={{ logData: null }}>
        <FormDataProvider initialFormData={exampleFormDataWithKeySecretAuth}>
          <StepReview onSubmit={jest.fn()} onEditClick={() => () => {}} />
        </FormDataProvider>
      </ApiContext.Provider>,
    );

    expect(await screen.findByText(/use single dynamodb table for state tracking/i)).toBeInTheDocument();
  });

  it('includes kinesis_single_table_state_tracking: true in the submit payload when enabled', async () => {
    asMock(fetch).mockReturnValueOnce(Promise.resolve({ title: 'Test', configuration: {} }));
    ...
    await userEvent.click(await screen.findByRole('button', { name: /complete cloudWatch setup/i }));

    await waitFor(() => expect(fetch).toHaveBeenCalledTimes(1));

    expect(fetch).toHaveBeenCalledWith('POST', expect.any(String), expect.objectContaining({
      kinesis_single_table_state_tracking: true,
    }));
  });

  it('includes kinesis_single_table_state_tracking: false in the submit payload when disabled', async () => {
    asMock(fetch).mockReturnValueOnce(Promise.resolve({ title: 'Test', configuration: {} }));

    const formDataWithDisabled = {
      ...exampleFormDataWithKeySecretAuth,
      awsKinesisSingleTableStateTracking: { value: false },
    };

    render(
      <ApiContext.Provider value={{ logData: null }}>
        <FormDataProvider initialFormData={formDataWithDisabled}>
          <StepReview onSubmit={jest.fn()} onEditClick={() => () => {}} />
        </FormDataProvider>
      </ApiContext.Provider>,
    );
    ...
    expect(fetch).toHaveBeenCalledWith('POST', expect.any(String), expect.objectContaining({
      kinesis_single_table_state_tracking: false,
    }));
  });
});
