...
jest.mock('components/collectors/fleets', () => ({ FleetDetail: () => <div>fleet detail content</div> }));
jest.mock('components/collectors/common', () => ({ CollectorsPageNavigation: () => <div>collectors nav</div> }));

const navigateTo = jest.fn();
jest.mock('react-router-dom', () => ({
  ...jest.requireActual('react-router-dom'),
  useParams: () => ({ fleetId: 'fleet-1' }),
...
describe('CollectorsFleetDetailPage', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('redirects to the overview wizard when collectors are not configured', () => {
    ...
    render(<CollectorsFleetDetailPage />);

    expect(navigateTo).toHaveBeenCalledWith('/system/collectors');
    expect(screen.queryByText('fleet detail content')).not.toBeInTheDocument();
  });
  ...
  it('renders the page once collectors are configured', () => {
    ...
    expect(screen.getByText('fleet detail content')).toBeInTheDocument();
