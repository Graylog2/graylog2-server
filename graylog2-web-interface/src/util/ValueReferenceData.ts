...
class Path {
  private path: string;
  private valueIsReference: boolean;
  private valueIsParameter: boolean;
  private getter: () => unknown;
  private setter: (value: unknown) => void;
  private parameterSetter: (name: string) => void;
  private valueTypeGetter: () => string;

  constructor(
    path: string,
    valueIsReference: boolean,
    valueIsParameter: boolean,
    getter: () => unknown,
    setter: (value: unknown) => void,
    parameterSetter: (name: string) => void,
    valueTypeGetter: () => string,
  ) {
    this.path = path;
...
export default class ValueReferenceData {
   ...
   */
  getPaths(): Record<string, Path> {
    return this.paths.toJS() as Record<string, Path>;
  }
  ...
  getData(): Record<string, unknown> {
    return this.data.toJS() as Record<string, unknown>;
  }

  /**
   * @private
   */
  private walkPaths(parentPath: Array<PathKey> = []): void {
    ...
    if (Map.isMap(data)) {
      ...
      (data as List<unknown>).toArray().forEach((_value: unknown, idx: number) => {
   ...
   * @private
   */
  private addPath(path: Array<PathKey>): void {
   ...
   * @private
   */
  private pathGetter(path: Array<PathKey>): () => unknown {
    return () => {
   ...
   * @private
   */
  private pathSetter(path: Array<PathKey>): (value: unknown) => void {
    return (value: unknown) => {
      if (ValueRefHelper.dataIsValueRef(this.data.getIn(path))) {
        this.data = this.data.setIn(path.concat(ValueRefHelper.VALUE_REF_VALUE_FIELD), value) as DataMap;
      } else {
        this.data = this.data.setIn(path, value) as DataMap;
      }
    };
  }
   ...
   * @private
   */
  private pathParameterSetter(path: Array<PathKey>): (name: string) => void {
    return (name: string) => {
      if (ValueRefHelper.dataIsValueRef(this.data.getIn(path))) {
   ...
   * @private
   */
  private pathType(path: Array<PathKey>): () => string {
    return () => {
      const data = this.data.getIn(path);

      if (ValueRefHelper.dataIsValueRef(data)) {
        return this.data.getIn(path.concat(ValueRefHelper.VALUE_REF_TYPE_FIELD)) as string;
      }
