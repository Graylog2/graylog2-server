...
type LookupTableActionsType = {
  ...
  getErrors: (
    ...
    dataAdapterNames: Array<string> | undefined,
  ) => Promise<unknown>;
...
export const LookupTablesActions = singletonActions('core.LookupTables', () =>
...
export const LookupTablesStore = singletonStore('core.LookupTables', () =>
  Reflux.createStore<LookupTablesStoreState>({
    ...
    create(table: LookupTable) {
      ...
      promise.catch(
        this._errorHandler('Creating lookup table failed', `Could not create lookup table "${table.name}"`),
      );
    ...
    update(table: LookupTable) {
      ...
      promise.catch(
        this._errorHandler('Updating lookup table failed', `Could not update lookup table "${table.name}"`),
      );
