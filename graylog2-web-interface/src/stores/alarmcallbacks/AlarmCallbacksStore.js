...
export const AlarmCallbacksStore = singletonStore(
  ...
  Reflux.createStore({
    ...
    list(streamId) {
      const failCallback = (error) =>
        UserNotification.error(
          `Fetching alert notifications failed with status: ${error.message}`,
    ...
    save(streamId, alarmCallback) {
      const failCallback = (error) => {
        const errorMessage =
          error.additional && error.additional.status === 400 ? error.additional.body.message : error.message;

        UserNotification.error(
          `Saving alert notification failed with status: ${errorMessage}`,
      ...
      promise.then(() => UserNotification.success('Alert notification saved successfully'), failCallback);
    ...
    delete(streamId, alarmCallbackId) {
      const failCallback = (error) =>
        UserNotification.error(
          `Removing alert notification failed with status: ${error.message}`,
      ...
      promise.then(() => UserNotification.success('Alert notification deleted successfully'), failCallback);
    ...
    update(streamId, alarmCallbackId, deltas) {
      const failCallback = (error) => {
        ...
        UserNotification.error(
          `Updating alert notification '${alarmCallbackId}' failed with status: ${errorMessage}`,
      ...
      promise.then(() => UserNotification.success('Alert notification updated successfully'), failCallback);
