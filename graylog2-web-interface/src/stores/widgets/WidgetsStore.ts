...
const WidgetsStore = Reflux.createStore({
    ...
    addWidget(dashboardId: string, widgetType: string, widgetTitle: string, widgetConfig: Object): Promise<string[]> {
        ...
        var url = URLUtils.qualifyUrl(jsRoutes.DashboardsApiController.addWidget(dashboardId).url);
    ...
    loadWidget(dashboardId: string, widgetId: string): Promise<string[]> {
        var url = URLUtils.qualifyUrl(jsRoutes.DashboardsApiController.widget(dashboardId, widgetId).url);
    ...
    updateWidget(dashboardId: string, widget: Widget): Promise<string[]> {
        var url = URLUtils.qualifyUrl(jsRoutes.DashboardsApiController.updateWidget(dashboardId, widget.id).url);
    ...
    loadValue(dashboardId: string, widgetId: string, resolution: number): Promise<string[]> {
        var url = URLUtils.qualifyUrl(jsRoutes.DashboardsApiController.widgetValue(dashboardId, widgetId, resolution).url);
    ...
    removeWidget(dashboardId: string, widgetId: string): Promise<string[]> {
        const url = URLUtils.qualifyUrl(jsRoutes.DashboardsApiController.removeWidget(dashboardId, widgetId).url);
