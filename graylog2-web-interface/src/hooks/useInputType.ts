...
const useInputType = (
  ...
  const { data, refetch, isLoading } = useQuery<InputDescription, Error>({
    queryKey: ['inputType', type],
    queryFn: () =>
      defaultOnError<InputDescription>(
        fetchInputType(type),
        'Loading input type failed with status',
        'Could not load input type',
      ),
    placeholderData: keepPreviousData,
    enabled,
  });

  return { data: data || undefined, refetch, isLoading };
};
