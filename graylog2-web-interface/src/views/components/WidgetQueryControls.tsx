...
export const updateWidgetSearchControls = (widget, { timerange, streams, queryString }) =>
  widget.toBuilder().timerange(timerange).query(createElasticsearchQueryString(queryString)).streams(streams).build();
          ...
          return (
            <Container>
              <PropagateDisableSubmissionState
                formKey="widget-query-controls"
                disableSubmission={disableSearchSubmit}
              />
                          ...
                          <PluggableCommands usage="widget_query">
                            {(customCommands) => (
                              <QueryInput
                                ...
                                streams={values?.streams}
                                placeholder={
                                  'Type your search query here and press enter. E.g.: ("not found" AND http) OR http_response_code:[400 TO 404]'
                                }
                                error={error}
                                ref={editorRef}
