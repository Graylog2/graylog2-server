...
type Props = {
  ...
  onAddEmptyElement: (elementKey: string) => void,
}

const AggregationElementSelect = ({ aggregationElements, onAddEmptyElement, formValues }: Props) => {
  const selectRef = useRef(null);
  const options = _getOptions(aggregationElements, formValues);

  const _onSelect = (elementKey: string) => {
    selectRef.current.clearValue();
    onAddEmptyElement(elementKey);
  };
