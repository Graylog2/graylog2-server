...
const updateColor =
  (rule: Rule, newColor: HighlightingColor, hidePopover: () => void) => async (dispatch: AppDispatch) =>
    dispatch(updateHighlightingRule(rule, { color: newColor })).then(hidePopover);
...
const HighlightingRule = forwardRef<HTMLDivElement, Props>(
  ({ rule, className, draggableProps, dragHandleProps }, ref) => {
