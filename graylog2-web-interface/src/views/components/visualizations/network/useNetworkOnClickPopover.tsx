...
import SankeyNodeActionsDropdown from 'views/components/visualizations/sankey/SankeyNodeActionsDropdown';
...
const useNetworkOnClickPopover = () => {
  ...
  const onChartClick = useCallback((_: OnClickMarkerEvent, e: PlotMouseEvent) => {
    const point = e.points?.[0] as unknown as NetworkPoint | undefined;
    ...
    const x = Number.isFinite(mouseEvent?.clientX) ? mouseEvent!.clientX : lastMouseRef.current.x;
    const y = Number.isFinite(mouseEvent?.clientY) ? mouseEvent!.clientY : lastMouseRef.current.y;
...
export default useNetworkOnClickPopover;
