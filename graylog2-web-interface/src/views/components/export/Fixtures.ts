...
const queries = [Query.builder().id('query-id-1').searchTypes([searchType]).build()];
...
export const messagesWidget = (id: string = 'widget-id-1') => MessagesWidget.builder().id(id).config(config).build();
const states: ViewStateMap = Immutable.Map({
  'query-id-1': ViewState.create(),
});
const searchWithQueries = Search.builder().id('search-id').queries(queries).build();
export const viewWithoutWidget = (viewType: ViewType) =>
  View.create().toBuilder().id('deadbeef').type(viewType).search(searchWithQueries).state(states).build();
// Prepare view with one widget
export const stateWithOneWidget = (widget) =>
  ViewState.builder()
    ...
    .titles(
      Immutable.Map<TitleType, Immutable.Map<string, string>>({
        widget: Immutable.Map<string, string>({ 'widget-id-1': 'Widget 1' }),
      }),
    )
    .build();
export const viewWithOneWidget = (viewType) =>
  viewWithoutWidget(viewType)
    .toBuilder()
...
const stateWithMultipleWidgets: ViewState = ViewState.builder()
  ...
  .titles(
    Immutable.Map<TitleType, Immutable.Map<string, string>>({
      widget: Immutable.Map({ 'widget-id-1': 'Widget 1', 'widget-id-2': 'Widget 2' }),
    }),
  )
  .build();
export const viewWithMultipleWidgets = (viewType) =>
  viewWithoutWidget(viewType)
    .toBuilder()
