...
const searchesUrl = qualifyUrl('/search/saved');

const fetchSearches = (searchParams: SearchParams) => {
...
const useSearches = (searchParams: SearchParams, { enabled }: Options = { enabled: true }): {
  data: {
  ...
  const { data, refetch, isFetching } = useQuery(
    ...
    {
      onError: (errorThrown) => {
        UserNotification.error(`Loading searches failed with status: ${errorThrown}`,
