...
const HighlightingRulesActions: HighlightingRulesActionsType = Reflux.createActions({
...
const HighlightingRulesStore = Reflux.createStore({
  ...
  onViewStateStoreChange({ state }: { state: ViewState }) {
    ...
    const rules = highlighting.reduce(
      (prev: StateType, rule: HighlightingRule) => prev.set(makeKey({ field: rule.field, value: rule.value }), rule.color),
