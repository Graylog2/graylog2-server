...
describe('FindNewActiveQuery', () => {
  ...
  it('returns current query when it is still present', () => {
    ...
    expect(FindNewActiveQuery(Immutable.List(['foo', 'bar', 'baz']), 'foo', Immutable.List(['bar', 'baz']))).toEqual(
      'foo',
    );
  });
  ...
  it('returns next query when current query is removed', () => {
    ...
    expect(FindNewActiveQuery(Immutable.List(['foo', 'bar', 'baz']), 'foo', Immutable.List(['foo', 'baz']))).toEqual(
      'bar',
    );
