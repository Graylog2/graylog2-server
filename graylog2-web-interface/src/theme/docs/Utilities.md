...
// or inside ThemeProvider

const Component = styled.div(({ theme }) => `
  color: ${theme.utils.readableColor(theme.colors.global.primary)};
`);
```
...
**`darkColor`**
  - defaults: Currently `colors.teinte.global.textDefault`

**`lightColor`**
  - defaults: Currently `colors.teinte.global.textAlt`
