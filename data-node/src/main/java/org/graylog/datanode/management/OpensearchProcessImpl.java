...
class OpensearchProcessImpl implements OpensearchProcess, ProcessListener {
    ...
    public OpensearchInfo processInfo() {
        return new OpensearchInfo(datanodeConfiguration.nodeName(), processState.getState(), isLeaderNode, getOpensearchBaseUrl().toString(), Optional.ofNullable(commandLineProcess).map(OpensearchCommandLineProcess::processInfo).orElse(ProcessInformation.empty()));
    }
