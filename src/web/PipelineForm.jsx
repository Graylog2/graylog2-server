...
const PipelineForm = React.createClass({
  ...
  _onSourceChange(value) {
    ...
    if (this.props.validatePipeline) {
      // have the caller validate the pipeline after typing stopped for a while. usually this will mean send to server to parse
      this.parseTimer = setTimeout(() => this.props.validatePipeline(pipeline, this._setParseErrors), 500);
    }
  },

  _onDescriptionChange(event) {
    const pipeline = this.state.pipeline;
    pipeline.description = event.target.value;
    this.setState({pipeline: pipeline});
  },
  ...
  render() {
    ...
    if (this.props.create) {
      triggerButtonContent = 'Create pipeline';
    } else {
    ...
    return (
      <span>
        ...
        <BootstrapModalForm ref="modal"
                            title={`${this.props.create ? 'Create' : 'Edit'} Processing Pipeline ${this.state.pipeline.title}`}
          ...
          <fieldset>
            ...
            <div style={{border: "1px solid lightgray", borderRadius: 5}}>
              <AceEditor
                ...
                value={this.state.pipeline.source}
                onLoad={this._onLoad}
