...
public class ElasticSearchIndexer extends Indexer {
    ...
    public ElasticSearchIndexer(Client elasticSearchClient, String indexName) {
    ...
    public boolean createIndex() throws IOException {
        ...
        ListenableActionFuture<CreateIndexResponse> response = builder.execute();
     ...
     */
    @Override
    public boolean bulkIndex(List<GELFMessage> messages, Set<String> types) {

        if (messages.isEmpty()) {
            return true;
        }

		Mapping.updateMapping(this.buildIndexURL(), types);
        ...
        for(GELFMessage gelfMessage : messages) {
            ...
            try {
                ...
                bulkRequest.add(elasticSearchClient.prepareIndex(indexName, gelfMessage.getFacility()).setSource(contentBuilder));
            } catch (IOException e) {
        ...
        if(futureResponse.isCancelled()) {
            ...
            if (bulkResponse.hasFailures()) {
            	...
            	if(LOG.isDebugEnabled()) {
            		LOG.debug(String.format("Successfully indexed %d messages", bulkResponse.items().length));
                }
            }
